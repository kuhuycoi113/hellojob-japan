...
import { Clock, Star, Briefcase, Sparkles, User, Send, Target, HelpCircle, ShieldCheck } from 'lucide-react';
...
export function AdvisorProfile({ advisorSlug }: { advisorSlug: string }) {
  ...
  const systemAdvantages = t.advisorProfile.systemAdvantages;
  const addedValue = t.advisorProfile.addedValue;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-1 space-y-8 lg:sticky lg:top-24">
            ...
            <Card className="shadow-lg">
                <CardHeader>
                    <CardTitle className="text-xl">{t.advisorProfile.professionalInfo.title}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="flex items-start gap-3">
                        <Clock className="w-5 h-5 text-muted-foreground mt-1 flex-shrink-0"/>
                        <div>
                            <p className="font-semibold text-sm">{t.advisorProfile.professionalInfo.experience.label}</p>
                     ...
                     <div className="flex items-start gap-3">
                        ...
                        <div>
                            <p className="font-semibold text-sm">{t.advisorProfile.professionalInfo.mainField.label}</p>
                     ...
                     <div className="flex items-start gap-3">
                        ...
                        <div>
                            <p className="font-semibold text-sm">{t.advisorProfile.professionalInfo.subField.label}</p>
            ...
            <Card className="shadow-lg text-center">
                <CardHeader>
                    <CardTitle className="text-xl">{t.advisorProfile.achievements.title}</CardTitle>
                </CardHeader>
                <CardContent className="flex justify-around">
                    <div>
                        <p className="text-4xl font-bold text-primary">{advisor.stats.successfulCandidates}</p>
                        <p className="text-sm text-muted-foreground">{t.advisorProfile.achievements.successfulCandidates}</p>
                    </div>
                     <div>
                        <p className="text-4xl font-bold text-primary">{advisor.stats.managedJobs}</p>
                        <p className="text-sm text-muted-foreground">{t.advisorProfile.achievements.managedJobs}</p>
                    </div>
                </CardContent>
            </Card>
          </div>
