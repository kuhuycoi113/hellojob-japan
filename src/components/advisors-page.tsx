...
import { Users, Phone, ArrowRight, MessageSquare } from 'lucide-react';
import Link from 'next/link';
import { translations } from '@/locales/translations';

const ZaloIcon = ({ className }: { className?: string}) => (
    <svg className={className} width="20" height="20" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {advisors.map((advisor, index) => {
            ...
            return (
              <Link key={index} href={`/advisors/${advisorSlug}`} className="group h-full">
                <Card className="flex flex-col text-center items-center p-6 rounded-xl shadow-lg h-full group-hover:shadow-2xl group-hover:-translate-y-2 transition-all duration-300">
                    ...
                    <CardFooter className="flex flex-col w-full gap-2 mt-auto pt-4">
                        <Button className="w-full h-12 pl-2 pr-4 rounded-full shadow-lg bg-sky-500 hover:bg-sky-600 text-white text-base flex items-center justify-center gap-2">
                            <div className="flex items-center gap-1">
                                ...
                                <span className="bg-white rounded-full p-1.5 h-8 w-8 flex items-center justify-center"><MessengerIcon /></span>
                                 <span className="bg-white rounded-full p-1.5 h-8 w-8 flex items-center justify-center"><MessageSquare className="w-5 h-5 text-primary" /></span>
                            </div>
                            <span className="whitespace-nowrap">{t.advisorProfile.contactButton}</span>
                        </Button>
                    </CardFooter>
                </Card>
              </Link>
